...
const getAllBlogs = async (query) => {
    const { page = 1, limit = 20, order_by = 'timestamp', state = 'published', ...filters } = query;
...
const getOneBlog = async (id, incrementReadCount = false) => {
    const query = BlogModel.findById(id).populate('description', 'first_name last_name');
    ...
    if (incrementReadCount) {
        ...
        const blog = await BlogModel.findByIdAndUpdate(id, { $inc: { read_count: 1 } }, { new: true })
            .populate('description', 'first_name last_name');
        return blog;
    } else {
        return query;
    }
