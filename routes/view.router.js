...
router.get('/', async (req, res) => {
    try {
        const blogs = await BlogService.getAllBlogs({ state: 'published', order_by: 'timestamp' });
...
router.get('/dashboard', requireAuth, async (req, res) => {
    try {
        ...
        const blogs = await BlogService.getOwnerBlogs(req.user.id, { order_by: 'timestamp' });
...
router.get('/blog/:id/edit', requireAuth, async (req, res) => {
    try {
        ...
        if (blog.author.toString() !== req.user.id) return res.status(403).send('Unauthorized');
